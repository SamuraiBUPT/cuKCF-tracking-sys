...
def process_scale(args):
    image, scaleFactors, base_width, base_height, currentScaleFactor, _roi, scale_model_width, scale_model_height, cell_size, s_hann, i = args

    patch_width = base_width * scaleFactors[i] * currentScaleFactor
    patch_height = base_height * scaleFactors[i] * currentScaleFactor
    ...
    if scale_model_width > im_patch.shape[1]:
        try:
            im_patch_resized = cv2.resize(
                im_patch, (scale_model_width, scale_model_height), None, 0, 0, 1)
        except:
            return None, i
    else:
        im_patch_resized = cv2.resize(
            im_patch, (scale_model_width, scale_model_height), None, 0, 0, 3)

    mapp = {'sizeX': 0, 'sizeY': 0, 'numFeatures': 0, 'map': 0}
    mapp = fhog.getFeatureMaps(im_patch_resized, cell_size, mapp)
    mapp = fhog.normalizeAndTruncate(mapp, 0.2)
    mapp = fhog.PCAFeatureMaps(mapp)

    totalSize = mapp['numFeatures'] * mapp['sizeX'] * mapp['sizeY']
    FeaturesMap = mapp['map'].reshape((totalSize, 1))
    FeaturesMap = s_hann[0][i] * FeaturesMap

    return FeaturesMap[:, 0], i
...
class KCFTracker:
    def __init__(self, hog=False, fixed_window=True, multi_scale=False):
        ...
        # all scale changing rate, from larger to smaller with 1 to be the middle
    ...
    def getFeatures(self, image, inithann, scale_adjust=1.):
        ...
        # 选取从原图中扣下的图片位置大小
        extracted_roi[2] = int(
            scale_adjust * self._scale * self._tmpl_sz[0] * self.currentScaleFactor)
        extracted_roi[3] = int(
            scale_adjust * self._scale * self._tmpl_sz[1] * self.currentScaleFactor)
    ...
    def update(self, image):
        ...
        # loc是中心相对移动量
        self._roi[0] = cx - self._roi[2] / 2.0 + loc[0] * \
            self.cell_size * self._scale * self.currentScaleFactor
        self._roi[1] = cy - self._roi[3] / 2.0 + loc[1] * \
            self.cell_size * self._scale * self.currentScaleFactor
        ...
        if self._multiscale:
            ...
            self.currentScaleFactor = self.currentScaleFactor * \
                self.scaleFactors[scale_pi[0]]
    ...
    def computeYsf(self):
        scale_sigma2 = (self.n_scales / self.n_scales **
                        0.5 * self.scale_sigma_factor) ** 2
    ...
    def dsstInit(self, roi, image):
        ...
        if self.base_width * self.base_height > self.scale_max_area:
            scale_model_factor = (self.scale_max_area /
                                  (self.base_width * self.base_height)) ** 0.5
        ...
        self.min_scale_factor = np.power(self.scale_step, np.ceil(np.log(
            (max(5 / self.base_width, 5 / self.base_height) * (1 + self.scale_padding))) / 0.0086))
        self.max_scale_factor = np.power(self.scale_step, np.floor(np.log(
            (min(image.shape[0] / self.base_width, image.shape[1] / self.base_height) * (1 + self.scale_padding))) / 0.0086))
    ...
    def get_scale_sample(self, image):
        ...
        for i in range(self.n_scales):
            ...
            patch_width = self.base_width * \
                self.scaleFactors[i] * self.currentScaleFactor
            patch_height = self.base_height * \
                self.scaleFactors[i] * self.currentScaleFactor
            ...
            if self.scale_model_width > im_patch.shape[1]:
                # print(im_patch.shape[1])
                if im_patch is None:
                    continue
                try:
                    im_patch_resized = cv2.resize(
                        im_patch, (self.scale_model_width, self.scale_model_height), None, 0, 0, 1)
                except:
                    continue
            else:
                im_patch_resized = cv2.resize(
                    im_patch, (self.scale_model_width, self.scale_model_height), None, 0, 0, 3)
    ...
    def train_scale(self, image, ini=False):
        ...
        if ini:
            ...
            # Get new A and new B
            self.sf_den = cv2.addWeighted(
                self.sf_den, (1 - self.scale_lr), new_sf_den, self.scale_lr, 0)
            self.sf_num = cv2.addWeighted(
                self.sf_num, (1 - self.scale_lr), new_sf_num, self.scale_lr, 0)
    ...
    def detect_scale(self, image):
        ...
        add_temp = cv2.reduce(complexMultiplication(
            self.sf_num, xsf), 0, cv2.REDUCE_SUM)

        # compute the final y
        scale_response = cv2.idft(complexDivisionReal(
            add_temp, (self.sf_den + self.scale_lambda)), None, cv2.DFT_REAL_OUTPUT)
...
def main():
    ...
    # 选择ROI
    roi = cv2.selectROI('tracking', first_frame,
    ...
    tracker.init([x, y, w, h], first_frame)
    onTracking = True
    ...
    while cap.isOpened():
        ...
        if onTracking:
            ...
            duration = t1 - t0
            fps = 1 / duration
            cv2.putText(frame, f'FPS: {fps:.2f}', (8, 20),
