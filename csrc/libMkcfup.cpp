...
Mat get_subwindow(const Mat &input, int cx, int cy, int width, int height, float currentScaleFactor) {
    ...
    cv::Rect border(-min(lefttop.x, 0), -min(lefttop.y, 0), max(rightbottom.x - (input.cols - 1), 0),
                    max(rightbottom.y - (input.rows - 1), 0));
    ...
    for (int n = 0; n < channels; ++n) {
        ...
        for (int i = 0; i < size1; ++i) {
            ...
            for (int j = 0; j < size2; ++j) {
                out[j] = (data1[i2[j]] - data1[i2[j] - region_size] - data2[i2[j]] + data2[i2[j] - region_size]) /
                         (region_area * maxval);
            }
        }
...
#pragma omp parallel for
    for (int i = 0; i < nScales; ++i) {
        ...
        input(roiRect).copyTo(subWindow);
        if (border != cv::Rect(0, 0, 0, 0))
            cv::copyMakeBorder(subWindow, subWindow, border.y, border.height, border.x, border.width,
                               cv::BORDER_REPLICATE);
        Mat out, temp;
        resize(subWindow, out, outsize);
        if (imchannel == 3) {
            cvtColor(out, temp, cv::COLOR_BGR2GRAY);
...
model newtrainmodel(complex_mat &alphaf_num1, complex_mat &alphaf_num2, complex_mat &alphaf_den1,
    ...
    complex_mat new_num1, new_num2, new_den1, new_den2, alphaf_num11, alphaf_num22, alphaf_den11, alphaf_den22,
        alphaf_num, alphaf_den;
    while (stop == 0) {
        ...
        if (frame == start_frame) {
            ...
            alphaf_num22 =
                plus_com(mul_com(alphaf_num2, (1 - learning_rate_hog)), mul_com(new_num2, learning_rate_hog));
            alphaf_den11 = plus_com(mul_com(alphaf_den1, (1 - learning_rate_cn)), mul_com(new_den1, learning_rate_cn));
            alphaf_den22 =
                plus_com(mul_com(alphaf_den2, (1 - learning_rate_hog)), mul_com(new_den2, learning_rate_hog));
            alphaf_num = plus_com(alphaf_num11, alphaf_num22);
            alphaf_den = plus_com(alphaf_den11, alphaf_den22);
        }
        ...
        if (count > 1) {
            ...
            if (sum(deltaalpha)[0] <= threshold * sum(abs(prevalpha))[0] &&
                (deltaD[0] + deltaD[1]) <= threshold * (prevD[0] + prevD[1]))
                stop = 1;
        }
        prevalpha = alpha;
        prevD[0] = p.d[0];
...
complex_mat resizeDFT2(const complex_mat &input, const int *sz) {
    ...
    resizeddft.img =
        Mat::zeros(sz[0], sz[1], CV_32FC1); // cout << resizeddft.real.rows << " " << resizeddft.real.cols << endl;
    // cout << imf.real << endl;
...
float inference(std::string &input_dir, std::string &output_dir, std::string &item) {
    ...
    // string sequences[] = {"Biker"};
    // int total_num = sizeof(sequences) / sizeof(string);
    // set the openmp set
    omp_set_num_threads(num_threads);
    float fps = 0;

    // in this loop, we will iterate each item in sequences list, and iterate their images.
    // load_video_info
    char buf[100];
    sprintf(buf, (output_dir + std::string("/results_") + item + std::string(".txt")).c_str());
    ...
    for (int i = 0; i < 4; ++i)
        init_gt[i] = 0;
    if (fp_gt == NULL)
        cout << "Cannot read the gt file." << endl;
    else {
    ...
    float output_sigma =
        sqrt(base_target_sz[0] / featureRatio * base_target_sz[1] / featureRatio) * output_sigma_factor;
    int use_sz[2];
    ...
    float *interpScaleFactors = new float[nScalesInterp], *interp_scale_exp = new float[nScalesInterp],
          *interp_scale_exp_shift = new float[nScalesInterp], *scaleSizeFactors_test = new float[nScales],
          *scaleSizeFactors = new float[nScales], *scale_exp = new float[nScales],
    ...
    if (use_dsst) {
        ...
        mega_scale_window =
            repeat(scale_window,
                   floor(scale_model_sz[1] / featureRatio) * floor(scale_model_sz[0] / featureRatio) * num_hog_fea, 1);
        mega_ysf.real =
            repeat(ysf.real,
                   floor(scale_model_sz[1] / featureRatio) * floor(scale_model_sz[0] / featureRatio) * num_hog_fea, 1);
        mega_ysf.img =
        ...
        max_scale_factor =
            pow(scale_step,
                floor(log(min(float(img.rows) / float(base_target_sz[0]), float(img.cols) / float(base_target_sz[1]))) /
                      log(scale_step)));
    }
...
#pragma omp parallel for
            ...
            if (imchannel != 3)
                zf_cn[0] = newfft2(z_cn[0], cos_window);
            ...
            // scale detection
            if (use_dsst) {
                Mat xs_pca = get_scale_subwindow(im, pos[1], pos[0], base_target_sz[1], base_target_sz[0],
                                                 scaleSizeFactors, scale_model_sz, currentScaleFactor, nScales,
                                                 featureRatio, num_hog_fea, num_threads);
                Mat temp_den = xs_pca;
                temp_den = temp_den.mul(mega_scale_window);

                dft(temp_den, kk, (cv::DFT_ROWS | cv::DFT_COMPLEX_OUTPUT));
                split(kk, channels);
                ...
                reduce(temp_sf_test.real, temp_scale_responsef_test.real, 0, cv::REDUCE_SUM, CV_32FC1);
                reduce(temp_sf_test.img, temp_scale_responsef_test.img, 0, cv::REDUCE_SUM, CV_32FC1);
...
#pragma omp parallel for
        ...
        for (int i = 0; i < num_pca_fea; ++i) {
            ...
            if (imchannel == 3)
                xf_cn[i] = newfft2(x_cn2[i], cos_window);
        }
        if (imchannel != 3)
            xf_cn[0] = newfft2(x_cn[0], cos_window);
        ...
        if (frame == start_frame) {
            ...
            p = newtrainmodel(alphaf_num1, alphaf_num2, alphaf_den1, alphaf_den2, d_num1, d_num2, d_den1, d_den2,
                              kf_hog, kf_cn, yf, frame, start_frame, learning_rate_cn, learning_rate_hog, y, imchannel,
                              lambda);
        } else if (frame % gap == 0) {
            Mat kf_hog = new_gaussian_correlation(xf_hog, xf_hog, hogsigma, 1, 1, imchannel, num_pca_fea);
            Mat kf_cn = new_gaussian_correlation(xf_cn, xf_cn, cnsigma, 1, 0, imchannel, num_pca_fea);
        ...
        // scale training
        if (use_dsst) {
            Mat xs_pca = get_scale_subwindow(im, pos[1], pos[0], base_target_sz[1], base_target_sz[0], scaleSizeFactors,
                                             scale_model_sz, currentScaleFactor, nScales, featureRatio, num_hog_fea,
                                             num_threads);
            if (frame == start_frame)
                s_num = xs_pca.clone();
            else
            ...
            dft(temp, kk, (cv::DFT_ROWS | cv::DFT_COMPLEX_OUTPUT));
            split(kk, channels);
            ...
            reduce(temp_sf_test.real, new_sf_den_test.real, 0, cv::REDUCE_SUM, CV_32FC1);
            reduce(temp_sf_test.img, new_sf_den_test.img, 0, cv::REDUCE_SUM, CV_32FC1);
            if (frame == start_frame) {
                sf_den.real = new_sf_den_test.real.clone();
                sf_den.img = new_sf_den_test.img.clone();
            } else
                sf_den =
                    plus_com(mul_com(sf_den, (1 - scale_interp_factor)), mul_com(new_sf_den_test, scale_interp_factor));
        }
        endTime = clock();
        ...
        // visualization
        if (visualization) {
            rectangle(im, cv::Point(pos[1] - target_sz[1] / 2, pos[0] - target_sz[0] / 2),
                      cv::Point(pos[1] + target_sz[1] / 2, pos[0] + target_sz[0] / 2), Scalar(255, 0, 0), 1, 1, 0);
